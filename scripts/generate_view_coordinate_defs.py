...
def generate_view_permutations() -> Iterable[ViewCoordinates]:
    ...
    D2 = ["Left", "Right"]
    D3 = ["Forward", "Back"]
    for i, j, k in itertools.product(D1, D2, D3):
...
def gen_rust_arch_decl() -> list[str]:
    key = "declarations"
    lines = [
        f"// {BEGIN_MARKER.format(key)}\n",
        f"// This section is generated by running `{SCRIPT_PATH} --rust`\n",
    ]
...
def gen_py_arch_decl() -> list[str]:
    ...
    lines = [
        f"# {BEGIN_MARKER.format(key)}\n",
        f"# This section is generated by running `{SCRIPT_PATH} --python`\n",
...
def gen_cpp_arch_decl() -> list[str]:
    ...
    lines = [
        f"// {BEGIN_MARKER.format(key)}\n",
        f"// This section is generated by running `{SCRIPT_PATH} --cpp`\n",
    ]
